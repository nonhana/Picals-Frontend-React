...
import { workDetailInfo, normalWorkList } from '@/test/data'
...
const WorkInfo: FC = () => {
  const [workInfo, setWorkInfo] = useState<WorkDetailInfo | null>(null)
  ...
  useEffect(() => {
    setLoading(true)
    setTimeout(() => {
      setWorkInfo(workDetailInfo)
    }, 1000)
    setTimeout(() => {
      setAuthorWorkList(normalWorkList)
    }, 2000)
    setLoading(false)
  }, [])

  return (
    <div className='relative w-175 flex flex-col items-center'>
      {/* 图片列表 */}
